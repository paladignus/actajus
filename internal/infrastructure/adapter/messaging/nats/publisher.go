...
func NewPublisher(config *config.NATSConfig, logger repository.Logger) (*Publisher, error) {
	conn, err := nats.Connect(
		...
		nats.DisconnectErrHandler(func(nc *nats.Conn, err error) {
			if err != nil {
				logger.Info(context.Background(), "[NATS] Disconnected", "error", err)
			}
		}),
		nats.ReconnectHandler(func(nc *nats.Conn) {
			logger.Info(context.Background(), "[NATS] Reconnected", "url", nc.ConnectedUrl())
		}),
	)
	if err != nil {
...
func (p *Publisher) Publish(ctx context.Context, evt event.IEvent) error {
	...
	p.logger.Info(ctx, "[NATS] Publishing event", "subject", subject, "event", evt)

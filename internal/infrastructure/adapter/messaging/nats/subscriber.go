...
func (p *Subscriber) Subscribe(ctx context.Context, eventName string, handler gateway.MessageHandler) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *Subscriber) handleMessage(ctx context.Context, msg *nats.Msg, handler gateway.MessageHandler) {

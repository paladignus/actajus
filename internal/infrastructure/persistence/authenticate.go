...
func (a Authenticate) SignIn(ctx context.Context, cpf string, password string) (dto.AuthenticatedOutput, error) {
	...
	sql := `SELECT DISTINCT p.idpeople, p.first_name, p.last_name, a.idaccounts FROM people p
	...
	err := a.db.Pool.QueryRow(ctx, sql, cpf, password).
		Scan(&autenticated.UserID, &autenticated.FirstName, &autenticated.LastName, &idaccount)

...
function enhanceForms() {
  ...
  const search = document.querySelector('input[type="search"][name="email"]');
  if (search && bootstrap.canManageSessions) {
...
function setupToasts() {
  ...
  const flashes = document.querySelectorAll(".dashboard-alert[data-flash-kind]");
  if (flashes.length === 0) {
    return;
  }
  flashes.forEach((flash, index) => {
    ...
    notifier({
      kind,
      source: flash.dataset.flashSource || void 0,
      code: flash.dataset.flashCode || void 0,
...
function toastTitle(kind, source) {
  switch (kind) {
    case "success":
...
function setupSidebarToggle() {
  ...
  groups.forEach((group) => {
    const submenuToggle = group.querySelector("[data-sidebar-submenu]");
    if (!submenuToggle) {
      return;
    }
...
function setupErrorRouteTools() {
  const searchRoot = document.querySelector("[data-error-route-search]");
  const results = document.querySelector("[data-error-route-results]");
  const input = document.querySelector("[data-error-route-input]");
  const recentRoot = document.querySelector("[data-error-recent-routes]");
  ...
  let activeIndex = -1;
  let currentItems = [];
  const renderLinks = (items, target, emptyMessage) => {
    target.innerHTML = "";
    if (items.length === 0) {
  ...
  const syncActiveRoute = () => {
    const links = Array.from(results.querySelectorAll(".error-card__shortcut"));
  ...
  const updateResults = (items, emptyMessage) => {
    currentItems = items;
  ...
  results.addEventListener("mousemove", (event) => {
    ...
    const link = target.closest(".error-card__shortcut");
    if (!link) {
      return;
    }
...
function setupCommandPalette() {
  ...
  const recent = overlay.querySelector("[data-command-palette-recent]");
  const openButtons = document.querySelectorAll("[data-command-palette-open]");
  const closeButtons = overlay.querySelectorAll("[data-command-palette-close]");
  ...
  const syncActive = () => {
    const links = Array.from(results.querySelectorAll(".command-palette__item"));
    links.forEach((link, index) => {
      const isActive = index === activeIndex;
  ...
  results.addEventListener("mousemove", (event) => {
    ...
    const link = target.closest(".command-palette__item");
    if (!link) {
      return;
    }
    const nextIndex = Number(link.dataset.routeIndex);
...
function setupSocialMediaForm() {
  const list = document.querySelector("[data-social-media-list]");
  const addButton = document.querySelector("[data-social-media-add]");
  const template = document.querySelector("#social-media-item-template");
  ...
  const bindRemoveHandlers = () => {
    const items = list.querySelectorAll("[data-social-media-item]");
    items.forEach((item) => {
      const removeButton = item.querySelector("[data-social-media-remove]");
      ...
      removeButton.addEventListener("click", () => {
        if (list.querySelectorAll("[data-social-media-item]").length <= 1) {
          const inputs = item.querySelectorAll('input[name="social_platform"], input[name="social_url"], input[name="social_media_id"]');
